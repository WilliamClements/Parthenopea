...
> ssTranspose                              = 0
>
> ssLead1, ssLead2, ssPicked, ssBass
>                        :: BandPart
> ssLead1                                  = makePitched Flute                ssTranspose          0         90
> ssLead2                                  = makePitched Flute                ssTranspose          0        115
> ssPicked                                 = makePitched AcousticGuitarNylon  ssTranspose          0         75
> ssBass                                   = makePitched ElectricBassPicked   ssTranspose          0        100
>
> ssailor                :: Music (Pitch, [NoteAttribute])
> ssailor                                  =
>     removeZeros
>     $ tempo                              ssTempo
>     $ keysig C Mixolydian
...
> slotTranspose                            = 0
>
> slotLead, slotStrum, slotBass, slotPerc
>                        :: BandPart
> slotLead                                 = makePitched Violin              slotTranspose 0                100
> slotStrum                                = makePitched AcousticGuitarNylon slotTranspose 0                100
> slotBass                                 = makePitched SynthBass1          slotTranspose 0                110
> slotPerc                                 = makeNonPitched                                                 100
>
> slot                   :: Int → Music (Pitch, [NoteAttribute])
> slot nn                                  =
>    removeZeros
>    $ tempo slotTempo
>    $ keysig G Dorian
...
> tcTempo                                  = 1
>
> tcLead, tcRepeat, tcBass, tcPerc
>                        :: BandPart
> tcLead                                   = makePitched OverdrivenGuitar        tcTranspose leadTranspose  100
> tcRepeat                                 = makePitched FrenchHorn              tcTranspose repeatTranspose 50
> tcBass                                   = makePitched FretlessBass            tcTranspose bassTranspose   85
> tcPerc                                   = makeNonPitched                                                 100
>
> basicLick :: Directives → Music (Pitch, [NoteAttribute])
> basicLick dives                          =
>   removeZeros
>   $ tempo                                tcTempo
>   $ keysig A Major
...
>   {-  4   -} , cs 6  (1/12 + qn), descendFrom dives tcLead (Cs, 6) Cs chromatic (qn*3)
>   {-  4   -} , tempo (5/4) tcVnt58
...
>   {-  7   -} , e 5 wn, ascendFrom dives tcLead (F,5) F chromatic dhn]
>
...
>   {-  4   -} , cs 5 wn
>   {-  2   -} , cs 5 qn, descendFrom dives tcLead (Cs,6) Cs chromatic qn]
>
...
>   {-  2   -} , tempo (7/4)   tcVnt02
>   {-  2   -} , tempo (7/4)   tcVnt03
>   {-  2   -} , ascendFrom dives tcLead (E,5) E chromatic hn
>        ]
>   tcV03C = -- measure 19
>     line [
...
>   tcV04F = -- measure 32
>     line [
>   {-  4   -}    cs 5 en, cs 5 sn, d 5 sn, descendFrom dives tcLead (E,5) E chromatic dhn
...
>   tcV05B = -- measure 34
>     line [
>   {-  6   -}   e 6 sn, descendFrom dives tcLead (E,6) E chromatic (en + ddqn), rest dqn, rest hn
...
>   tcV08D = -- measure 59
>     line [
>   {-  4   -}   tempo (5/4) (addDur en [d 5, e 5, f 5, e 5, d 5]), descendFrom dives tcLead (E,5) E chromatic hn
...
>   {-  5.3 -} , cs 6 ((1/12) + hn), descendFrom dives tcLead (Cs, 6) Cs chromatic qn, rest hn
...
> dhTranspose                              = 3
>
> dhVibe, dhLead, dhBass, dhOrgn, dhSynh, dhPerc
>                        :: BandPart
> dhVibe                                   = makePitched Vibraphone            dhTranspose      0         75
> dhLead                                   = makePitched Violin                dhTranspose      0        100
> dhBass                                   = makePitched ElectricBassPicked    dhTranspose      0         75
> dhOrgn                                   = makePitched ReedOrgan             dhTranspose      0         90
> dhSynh                                   = makePitched FrenchHorn            dhTranspose      0        100
> dhPerc                                   = makeNonPitched                                              100
>
...
>
> deathlessHorsie        :: Directives → Music (Pitch, [NoteAttribute])
> deathlessHorsie dives                    =
>   removeZeros
>   $ tempo dhTempo
...
>                       , t32 [gs 5 qn, fs 5 en], descendFrom dives dhLead (Fs, 5) Fs chromatic hn]
> -- 2/4
...
>
>     l025        = line [ds 5 hn, chord [line [grace (-1) (e 6 qn), descendFrom dives dhLead (E, 6) E chromatic qn]
>                                ,        line [grace (-2) (b 5 qn), descendFrom dives dhLead (B, 5) B chromatic qn]]
...
>          , grace (-2) (fs 5 en), descendFrom dives dhLead (Fs, 5) Fs chromatic qn]
>     l048        =
...
>           , cs 5 qn, descendFrom dives dhLead (Cs, 5) Cs chromatic qn]
>
...
>           , descendFrom dives dhLead (Fs, 5) Fs chromatic qn, grace (-5) (b 5 dqn)]
>     l060        =
...
>           , chord [line [grace (-4) (gs 5 qn), rest qn], line [cs 6 qn, ascendFrom dives dhLead (Cs, 6) Cs chromatic qn]]
...
>     l099        =
>       line [rest en, chord [line [fs 4 en, fs 4 qn, descendFrom dives dhLead (Fs, 4) Fs chromatic qn]
>                           , line [ b 4 en,  b 4 qn, descendFrom dives dhLead ( B, 4) B chromatic qn]
>                           , line [rest en, ds 5 qn, descendFrom dives dhLead (Ds, 5) Ds chromatic qn]]
...
>
> pgLead                 :: BandPart
> pgLead                                   = makePitched ElectricGuitarClean               pgTranspose     0     100
> pgBass                 :: BandPart
> pgBass                                   = makePitched SynthBass1 pgTranspose     0     100
> pgPerc                 :: BandPart
> pgPerc                                   = makeNonPitched                                         100
>
> packardGoose           :: Directives → Music (Pitch, [NoteAttribute])
> packardGoose dives                       =
>   removeZeros
>   $ tempo pgTempo
...
>           , line [fs 4 en, times 4 (fs 4 sn), fs 4 en, ascendFrom dives pgLead (F,4) F chromatic hn]
...
>     lead202_204 =
>       line [line [e 3 en, fs 3 en, ascendFrom dives pgLead (Fs, 3) Fs chromatic hn]
...
>
> hgLead                 :: BandPart
> hgLead                                   = makePitched Accordion                         hgTranspose     0     100
> hgBass                 :: BandPart
> hgBass                                   = makePitched SynthBass1                        hgTranspose     0     100
> hgPerc                  :: BandPart
> hgPerc                                   = makeNonPitched                                                      100
>
> greenMore              :: Music (Pitch, [NoteAttribute])
> greenMore                                =
>   removeZeros
>   $ tempo hgTempo
>   $ chord [leadMusic, bassMusic, percMusic]
